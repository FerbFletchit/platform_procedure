	...
	[1] = function( Platform ) -- Everything controlled here.
		...
		Platform["Callback"] = function( Platform ) -- What happens when we reach our destination.
			...
			if SERVER then
				...
				local Shifter = math.random(1,10)

				if Shifter == 1 then
					timer.Simple( math.random(30,80), function()
						if IsValid(Platform) then
							Platform:EmitSound("physics/metal/metal_solid_strain"..math.random(1,2)..".wav",75,math.random(90,100),1,CHAN_AUTO)
							Platform["Travel"]["maxangular"] = 10
							Platform["Travel"]["maxangulardamp"] = 100
							Platform["Travel"]["angle"] = Angle(0,90*math.random(1,6),0)
						end
					end )
				else
					Platform:PhysicsInit(SOLID_VPHYSICS)
				...
				if IsValid(Platform) then
					...
					if IsValid( Platform["Bridge"] ) then
						Platform["Bridge"]:SetModel("models/aceofspades/prefabs/super_bridge.mdl")
						
						Platform["Bridge"]:SetPos(Platform:GetPos()+Platform:GetUp()*30)
