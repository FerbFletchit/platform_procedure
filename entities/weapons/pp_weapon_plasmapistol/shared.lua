...
SWEP.WorldModel = "models/aceofspades/weapons/autopistol.mdl"

SWEP.ShowViewModel = false
SWEP.ShowWorldModel = false

SWEP.VElements = {
	["pp_plasmapistol"] = { type = "Model", model = "models/aceofspades/weapons/autopistol.mdl", bone = "ValveBiped.clip", rel = "", pos = Vector(0.518, 3.635, 4.675), angle = Angle(-90, 90, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["pp_plasmapistol"] = { type = "Model", model = "models/aceofspades/weapons/autopistol.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(5.714, -1, 3.635), angle = Angle(180, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Primary.TakeAmmo = 0 -- Infinite
...
SWEP.IronSightsPos      = Vector( -7.6, -0, -3 )
SWEP.IronSightsAng      = Vector( 4, -5, 4 )
...
function SWEP:PreDrawViewModel(vm, wep, ply)
	--[[if (IsValid(vm)) then
		local atch = vm:GetAttachment(1)
		local pos, ang = atch.Pos, atch.Ang
		ang:RotateAroundAxis(ang:Right(), 90)

		local Bar_Width = math.Round(math.Clamp((140)*(self:GetNextPrimaryFire() - CurTime()),0,140))
		if Bar_Width == 0 then
			Bar_Width = 140
		end
		
		cam.Start3D2D(pos - ang:Right() * 10, ang, 0.05)
			draw.RoundedBox(0,0,0,150,30,Color(0,0,0,130))
			draw.RoundedBox(0,5,5,Bar_Width,20,self:GetColor())
			--draw.SimpleText("Weapon Quality: ".."Gold","PP_Small",0,0,Color(255,255,255),TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
			--draw.SimpleText("Weapon Damage: "..self.Primary.Damage,"PP_Small",0,25,Color(255,255,255),TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
		cam.End3D2D()
	end]]
end
...
function SWEP:PrimaryAttack()
	...
	// Play shoot sound
	self.Weapon:EmitSound( self.Primary.Sound, PP["Weapon_Decibel"], 80, 1, CHAN_AUTO )
