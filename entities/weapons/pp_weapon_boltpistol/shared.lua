...
SWEP.WorldModel = "models/aceofspades/weapons/pistol.mdl"

SWEP.ShowViewModel = false
SWEP.ShowWorldModel = false

SWEP.VElements = {
	["pp_plasmapistol"] = { type = "Model", model = "models/aceofspades/weapons/pistol.mdl", bone = "ValveBiped.clip", rel = "", pos = Vector(0.518, -0.519, 2.596), angle = Angle(90, -180, 90), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["pp_plasmapistol"] = { type = "Model", model = "models/aceofspades/weapons/pistol.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.675, -0.96, -3.519), angle = Angle(0, 0, 180), size = Vector(1.399, 1.399, 1.399), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
--[[		self.Primary.ChanceofJam = (1) / PP["Ingots"][1]["Swep_Multiplier"]
		self.Primary.Damage = self.Primary.Damage*Qual
		self.Primary.Delay = self.Primary.Delay + ( self.Primary.Delay / Qual )
...
function SWEP:DrawHUD()
	if IsValid(self) then
		if self.Weapon.CurrentlyJammed then
			draw.SimpleText("WEAPON JAMMED","PP_Small",ScrW()/2,ScrH()/2-50,Color(255,0,0),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	end
end

function SWEP:PrimaryAttack()
	--[[if self.Weapon.CurrentlyJammed then return end
	self.Weapon:SetNextSecondaryFire( CurTime() + self.Primary.Delay )
	self.Weapon:SetNextPrimaryFire( CurTime() + self.Primary.Delay )

	local PP_ChanceofJam = math.Rand(1,10)
	print(PP_ChanceofJam, self.Primary.ChanceofJam)
	
	if PP_ChanceofJam < self.Primary.ChanceofJam then


		timer.Simple(1, function()
			if IsValid(self.Weapon) then
				self.Weapon.CurrentlyJammed = false
			end
		end )

		self.Weapon.CurrentlyJammed = true
	else]]
