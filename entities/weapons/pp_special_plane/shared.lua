...
SWEP.WorldModel = "models/aceofspades/tools/jetpack2.mdl"

SWEP.ShowViewModel = false
SWEP.ShowWorldModel = false

SWEP.VElements = {
	["pp_special_plane"] = { type = "Model", model = "models/aceofspades/tools/jetpack2.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.596, 3.635, -2.597), angle = Angle(-45.584, -22.209, -10.52), size = Vector(0.3, 0.3, 0.3), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/aceofspades/tools/jetpack2.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(5.714, 0, -1.558), angle = Angle(-38.571, -8.183, 90), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.ViewModelBoneMods = {
	...
	["ValveBiped.Bip01_R_Hand"] = { scale = Vector(1.016, 1.016, 1.016), pos = Vector(0, 0, 0), angle = Angle(43.333, 5.556, 0) },
...
function SWEP:PrimaryAttack()
	if SERVER then
		local flying_plane  = ents.Create("prop_physics")
		
		if IsValid(flying_plane) then
			flying_plane:SetModel("models/aceofspades/tools/jetpack2.mdl")
			flying_plane:SetModelScale(0.6)
			flying_plane:SetPos(self.Weapon:GetPos()+self.Weapon:GetUp()*10)
			flying_plane:SetAngles( Angle(-45.584, -22.209, -10.52) ) -- Angle(-45.584, -22.209, -10.52)
			flying_plane:Spawn()

			flying_plane:SetMaterial(self.VElements["pp_special_plane"]["material"])
			flying_plane:SetColor(self.VElements["pp_special_plane"]["color"])

			local PhysicsObject = flying_plane:GetPhysicsObject()
			if IsValid(PhysicsObject) then
				PhysicsObject:SetMass(10)
				PhysicsObject:AddVelocity(self.Owner:GetForward()*6000)
			end

			local trail = util.SpriteTrail( flying_plane, 0, Color( 255, 255, 255 ), false, 15, 1, 4, 1 / ( 15 + 1 ) * 0.5, "trails/laser" )

			timer.Simple(2, function()
				if IsValid(flying_plane) then
					flying_plane:EmitSound("pp_sound_effects/death.mp3",100,100,1,CHAN_AUTO)		
					
					for key, value in pairs(ents.FindInSphere( flying_plane:GetPos(), 500 ) ) do	
						
						if IsValid(value) and value:IsNPC() then
							
							value:SetVelocity( Vector(math.random(-500,500), math.random(-500,500), math.random(-500,500) ) )
					...
					PP_ActionEffect(flying_plane, "pp_impact", 2)
