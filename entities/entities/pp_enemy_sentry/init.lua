...
--util.AddNPCClassAlly(CLASS_MILITARY,"monster_sentry")
ENT.sModel = "models/half-life/sentry.mdl"
ENT.fViewAngle = 360
ENT.fViewDistance = 600
ENT.fHearDistance = 0
ENT.AIType = 1
...
function ENT:TurnOn(fcDone)
	...
	self:EmitSound(self.sSoundDir .. "tu_alert.wav", 40, math.random(70,120))
...
function ENT:OnThink()
	...
	local bPos = self:LocalToWorld(Vector(0.2688, -0.0289, 47.4655)  +Vector(0,0,fDist /self.fViewDistance *30))
...
function ENT:EventHandle(...)
	...
	if (event == "rattack") then
		...
		self:EmitSound(self.sSoundDir .. "tu_fire1.wav", 65, 100 )
		...
		tblBullet.Tracer = 1
		tblBullet.Force = 6
		tblBullet.Damage = math.random(1,2)
